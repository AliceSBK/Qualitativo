...
O card "Base de processos" agora recalcula de verdade sete categorias a partir da planilha bruta importada (mesmo formato usado para montar o snapshot atual: colunas como `nr_pasta`, `nr_processo`, `ds_causa_raiz`, `ds_produto`, `ds_situacao`, `dt_entrada`, `nm_escritorio`, `nm_advogado_do_autor`, `nm_analista_interno`, `ds_orgao`, `ds_estado`): **A Classificar**, **Venda Enganosa**, **MBA pós 04/07**, **Marco A. Peixoto**, **Cleyton S. Barbosa**, **Órgão (CNJ)** e **Escritórios**. As regras foram validadas comparando a planilha bruta contra exportações reais do próprio sistema, com correspondência de 98% a 100%, dependendo da categoria.
...
Em "Escritórios", o erro não é o escritório em si (ele não muda), é a analista interna atribuída ao caso: cada escritório tem uma responsável fixa (por exemplo, MBA Advogados é sempre Samuel Ribeiro da Silva, Renno Machado e Lima e Feigelson são sempre Gabriela Silva Oliveira), validado contra a base real com mais de 98% de concentração numa única analista por escritório. UV Advogados e E&S Advogados ficam de fora dessa regra de propósito: têm mais de uma analista com presença estável mês a mês, o que indica divisão legítima de carteira, não erro.

As demais categorias (**Bancoob**, **Cessão de Crédito**, **Campos Vazios**, **Superendividamento/Estratégico >200k**) dependem de tabelas de roteamento que não estão presentes na planilha de processos, só na planilha bruta é possível ver o resultado já aplicado, não a regra. Elas continuam com os valores da última importação até que a regra exata seja informada ou seja possível portar o processo que já gera esses números hoje.
